...
public class Three implements Screen {
    ...
    @Override
    public void show() {
        Texture logoTexture = new Texture(Gdx.files.internal("Three.png"));
        ...
        Tween.to(logoScreen, SpriteAccessor.ALPHA, 1).target(0.5f).repeatYoyo(1, 0.5f).setCallback(new TweenCallback() {
            @Override
            public void onEvent(int type, BaseTween<?> source) {
                ((Game)Gdx.app.getApplicationListener()).setScreen(new Two());
            }
        }).start(tweenManager);
    }
